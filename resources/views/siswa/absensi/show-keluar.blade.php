                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered text-nowrap">
                                ...
                                <tbody>
                                    <tr>
                                        <td>jam {{ Str::substr($absensi_karyawan->absen_keluar, 11, 5) }}</td>
                                        <td>
                                            {!! (strtotime($absensi_karyawan->absen_keluar) < strtotime($absensi->jam_keluar)) ? '<span class="badge badge-danger">Keluar Belum saatnya</span>' : '<span class="badge badge-primary">Sukses</span>' !!}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
...
<script>
    @if ($absensi_karyawan->izinkan == null)
        @if ($absensi_karyawan->absen_keluar == null)
        var lastResult, countDownDate = new Date("{{ $absensi->tgl }} {{ $absensi->jam_masuk }}").getTime(),
        x = setInterval(function() {
            var e = (new Date).getTime();  
            // e = countDownDate - e;
            // Math.floor(e / 864e5), Math.floor(e % 864e5 / 36e5), Math.floor(e % 36e5 / 6e4), Math.floor(e % 6e4 / 1e3);
            e > 0 && (clearInterval(x), $("#qr-reader").css("display", "block"), $(".waktu").css("display", "none"), new Html5QrcodeScanner("qr-reader", {
                fps: 10,
                qrbox: 250
